    ...
    reader.onload=()=>{
      ...
      img.onload=()=>{
        ...
        if(pathCount>PATH_LIMIT){
          report("pdf",0.95);
          // Drop out of the vector path entirely — no svg2pdf call at all.
        ...
        // Hard deadline: if svg2pdf hasn't finished in 30s, fall back to the
        ...
        <div style={{padding:"8px 14px",display:"flex",alignItems:"center",gap:6,background:"#1a1a1a",borderBottom:"1px solid rgba(255,255,255,0.1)",flexShrink:0,flexWrap:"wrap"}}>
          ...
          {(()=>{
            ...
            return <div style={{position:"relative"}} onMouseEnter={()=>{clearTimeout(dvColorTimer.current);setShowDvColorMenu(true);}} onMouseLeave={()=>{dvColorTimer.current=setTimeout(()=>setShowDvColorMenu(false),250);}}>
              ...
              {showDvColorMenu&&<div onMouseEnter={()=>clearTimeout(dvColorTimer.current)} onMouseLeave={()=>{dvColorTimer.current=setTimeout(()=>setShowDvColorMenu(false),250);}} style={{position:"absolute",top:"100%",left:"50%",transform:"translateX(-50%)",marginTop:4,background:"#2a2a2a",border:"1px solid rgba(255,255,255,0.12)",borderRadius:8,padding:8,zIndex:100,boxShadow:"0 4px 12px rgba(0,0,0,0.4)",display:"flex",gap:6,whiteSpace:"nowrap"}}>
                {COLORS.map(c=>(
              ...
              {helpTab==="hosting"&&(
                <div>
                  ...
                  <div style={{background:"rgba(52,168,83,0.08)",border:"1px solid rgba(52,168,83,0.2)",borderRadius:12,padding:16,marginBottom:20}}>
                    ...
                    <div style={{fontSize:12,color:"rgba(255,255,255,0.5)",lineHeight:1.7}}>SiteShrimp is designed for self-hosting. You control your backend, storage, AI, language, and reporting. No vendor lock-in, no recurring fees. 23 languages built in.</div>
                  </div>
                  ...
                  {(()=>{
                    ...
                    return React.createElement(React.Fragment,null,
                      ...
                      React.createElement(Section,{icon:"🌐",title:"LANGUAGE — 22 LANGUAGES BUILT IN",color:"#00bcd4"},
                        React.createElement("div",{style:{fontSize:12,color:"rgba(255,255,255,0.5)",lineHeight:1.7,marginBottom:12}},"The entire app is translated into 23 languages — UI labels, buttons, dropdown options (158 components, 296 issues, levels, zones, rooms, durations, costs), and email reports. Switch anytime from Settings → Language."),
                        React.createElement("div",{style:{display:"flex",flexDirection:"column",gap:6}},
              ...
              {helpTab==="features"&&(
                <div>
                  {(()=>{const fc=[
                    ...
                    ["Review",["Full-text search with highlighting","AI natural language search (voice + text)","Filter by status, severity, entry type","Collapsible filters with clear button","Batch update (Status, Severity, Assignee, Duration, Target Date, Shared Comment)","Batch delete (Admin only)","LIST / MAP view toggle for GPS-pinned entries","Map view: tap a pin → slide-up preview card at the rear of the master map (keeps spatial context)","Map view: horizontal pin chip strip lists every pinned entry for quick navigation","Map view: batch select pins or chips, then apply shared status / severity / comment / delete","Entry type badges on list & detail (translated)","Detail view with all fields + photos","Update status workflow (5 stages)","Verification photo on Close / Verify","Before / after photo comparison slider","Resolution timeline (visual, color-coded)","Photo comments in timeline","Markup on comment photos (tap to annotate)","Edit own comments inline (with edited indicator)","Quick reactions (thumbs, check, warn, fix)","Delete entry (Admin only)","Telegram alerts on new entry, status change, bulk updates & bulk comments"]],
...
"GPS-tag map thumbnails + consolidated pin overview embedded in PDF exports (reliable OSM tile-stitch fallback when no Google key)","Pins render in their severity colors regardless of section toggles","Live progress feedback during PDF export (Preparing → Rendering drawing N/M → Saving)","Per-drawing PDF export from the viewer"]],
                    ...
                    ["Setup & Integrations",["Single Settings dropdown for all one-time setup","AI multi-provider setup + test (Gemini, Ollama, OpenAI)","Telegram bot setup + test","Storage mode selector (PocketBase, local path, Google Drive)","Language selector (23 languages)","Daily AI usage limit","Email report config (inside Report tab)","Green ✓ check per configured integration"]],
